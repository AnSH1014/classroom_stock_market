...
let gameState = {
  ...
  speed: 2,
  volatility: 2,
  newsText: '시장 개장을 기다리고 있습니다...',
};

// students: { socketId -> { name, cash, holdings:{stockId->qty} } }
const students = {};
...
function endGame() {
  ...
  io.emit('game_ended', {
    stocks: STOCKS.map(s => ({ id:s.id, emoji:s.emoji, name:s.name, price:s.price })),
    leaderboard: buildLeaderboard(),
    newsText: gameState.newsText,
  });
}
...
io.on('connection', (socket) => {
  ...
  socket.on('student_join', (name) => {
    if (!name || students[socket.id]) return;
    students[socket.id] = { name: name.trim().slice(0,8), cash: INITIAL_CASH, holdings: {} };
    socket.emit('joined', {
  ...
  socket.on('buy', ({ stockId, qty }) => {
    ...
    if (student.cash < cost) {
      socket.emit('error_msg', '잔액이 부족해요!');
      return;
    }
    student.cash -= cost;
  ...
  socket.on('sell', ({ stockId, qty }) => {
    ...
    if (held < qty) {
      socket.emit('error_msg', `보유 주식이 ${held}주뿐이에요!`);
      return;
    }
  ...
  // Teacher: volatility
  socket.on('teacher_vol', (v) => {
    gameState.volatility = Math.max(1, Math.min(5, v));
  });
  ...
  socket.on('teacher_news', ({ text, targets, effect, random }) => {
    ...
    if (random) {
      STOCKS.forEach(s => {
        ...
        s.price = Math.max(50, Math.round(s.price * (1 + r)));
        s.history.push(s.price);
      });
    } else {
      (targets || []).forEach(tid => {
        ...
        if (s) {
          s.price = Math.max(50, Math.round(s.price * (1 + effect)));
          s.history.push(s.price);
        }
      });
    }
  ...
  socket.on('teacher_reset', () => {
    ...
    gameState.newsText = '시장 개장을 기다리고 있습니다...';
    clearInterval(timerInterval);
    clearInterval(priceInterval);
    STOCKS.forEach(s => {
      s.price = INITIAL_PRICES[s.id];
      s.history = [s.price];
    });
    Object.keys(students).forEach(id => {
      students[id].cash = INITIAL_CASH;
